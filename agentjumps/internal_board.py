...
class InternalBoard:
    ...
    def eval(self) -> float:
        """
        Evaluación modificada: el agente intenta posicionarse para ser saltado.
        """
        if self.terminal_state():
            if self.board.winner_color == self.player_color.opponent:
                return np.inf  # Queremos que el oponente gane
            elif self.board.winner_color == self.player_color:
                return -np.inf

        score = 0

        # Penaliza si estamos lejos del enemigo
        for p_coord in self.player_coords:
            min_dist = min(abs(p_coord.r - e_coord.r) + abs(p_coord.c - e_coord.c) for e_coord in self.enemy_coords)
            score -= min_dist

        # Penaliza si estamos en esquinas o zonas no accesibles
        for coord in self.player_coords:
            if coord.r in [0, constants.BOARD_N - 1] or coord.c in [0, constants.BOARD_N - 1]:
                score -= 5

        # Bonifica si estamos en línea con más ranas (potenciales cadenas)
        score -= 2 * self.grouping_penalty(self.player_coords)
        #score += 10 * self.make_enemy_jumpable()
        return score


    def make_enemy_jumpable(self):
        """
        Retorna una puntuación alta si el enemigo puede saltar nuestras ranas.
        """
        score = 0
        for enemy in self.enemy_coords:
            for d in Direction:
                over = enemy + d
                landing = over + d
                if self._coord_within_bounds(over) and self._coord_within_bounds(landing):
                    if self.board._cell_occupied(over) and self.board._cell_empty(landing):
                        score += 1
        return score
    
    def grouping_penalty(self, coords):
        from collections import Counter
        rows = [coord.r for coord in coords]
        freq = Counter(rows)
        penalty = sum((v - 1) for v in freq.values() if v > 1)
        return penalty
